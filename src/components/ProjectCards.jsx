...
const ProjectCard = ({ project }) => {
  return <Card isBlurred className=" w-full lg:h-full">
    <CardHeader className="flex gap-3">
      <a href={project.link}>
      <h1 className="text-3xl font-sans font-bold text-blue-500 drop-shadow-lg">{project.name}</h1>
      </a>
    </CardHeader>
    <Divider />
    <CardBody>
      ...
      {project.link?<a href={project.link}>
        <p className="text-lg font-bold text-white underline font-sans px-4">Project Code</p>
      </a>: <div></div>}
      {project.link2? 
      <a href={project.link2}>
        <p className="text-lg font-bold text-lime-400 underline font-sans px-4">Live Demo</p>
      </a>
      : <div></div>}
      </div>
      ...
      <div className="flex flex-wrap justify-center items-center w-full">
        {project.badges.map((badge) => (
          <div key={badge}>
            ...
            <p className="text-md font-bold text-blue-100 mx-2 my-1">{badge}</p>
            </Card>
          </div>
        ))}
      </div>
